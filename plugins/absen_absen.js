...
let handler = async (m, { conn, groupMetadata, usedPrefix }) => {
    ...
    if (!(id in conn.absen)) {
        await conn.sendButton(m.chat, bottime, `*Tidak Ada Absen Berlangsung*`, `${'𝐒𝐓𝐀𝐑𝐓 𝐀𝐁𝐒𝐄𝐍𝐓'}`, [
...
${cmenua}
`
await conn.sendButton(m.chat, caption, `${'Absen'}`, [['𝐇𝐀𝐃𝐈𝐑', `${usedPrefix}absen`], ['𝐀𝐁𝐒𝐄𝐍𝐓 𝐋𝐈𝐒𝐓', `${usedPrefix}cekabsen`]], m, { mentions: conn.parseMention(caption) })
