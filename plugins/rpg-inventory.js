...
*┠─────⎔ ITEMS*
${items}
┇• Total Item: ${Object.keys(inventory.items).map(v => user[v]).reduce((a, b) => a + b, 0)} Items` : ''}${crates ? `
┇
*┠─────⎔ CRATE*
${crates}
┇• Total Crate: ${Object.keys(inventory.crates).map(v => user[v]).reduce((a, b) => a + b, 0)} Crates` : ''}${pets || user.petFood ? `
┇
*⎔─────⎔ PEETS*
${pets ? pets + '\n' : ''}${user.petFood ? '🍖 Peet Food: ' + user.petFood : ''}` : ''}${cooldowns ? `
┇
┖──────────────────⎔*
...
`.trim()
  conn.sendButton(m.chat, `*${htki} INVENTROY ${htka}*`, caption, null, [[`${user.health < 60 ? '𝗛𝗘𝗔𝗟': '𝗔𝗗𝗩𝗘𝗡𝗧𝗨𝗥𝗘'}`,`${user.health < 60 ? '.heal': '.adventure'}`],['𝗣𝗥𝗢𝗙𝗜𝗟𝗘','.pp']],m)
}
