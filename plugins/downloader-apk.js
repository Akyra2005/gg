...
let handler = async (m, { conn, args, usedPrefix, command }) => {
	if (!args[0]) throw `*Format: ${usedPrefix + command} https://play.google.com/store/apps/details?id=com.linecorp.LGGRTHN*`
	let res = await apkDl(args[0])
	await m.reply('*In Download Process, Please Wait...*')
